...
- [ ] **Step 2: session.py 注册模型**

在 `init_db()` 的 imports 列表末尾添加 `JDDiagnosisV2` 的 import：
...
def build_jd_evaluator_messages(profile_json: str, jd_extract_json: str) -> list[dict[str, str]]:
    return [
...
职责：接收 JD 文本，调用 LLM，返回 `JDExtract`。
...
def parse_jd(jd_text: str) -> JDExtract:
    ...
    if not jd_text or len(jd_text.strip()) < 10:
        logger.warning("JD 文本过短，跳过解析: len=%d", len(jd_text or ""))
        return JDExtract()
...
```bash
python -c "from backend2.services.jd.parser import parse_jd; print('OK')"
```
...
```bash
git add backend2/services/jd/parser.py
git commit -m "feat(backend2): add JD parser layer"
```
...
def evaluate(profile: ProfileData, jd: JDExtract) -> JDDiagnosisResult:
    ...
    messages = build_jd_evaluator_messages(profile_json, jd_extract_json)
...
```bash
python -c "from backend2.services.jd.evaluator import evaluate; print('OK')"
```
...
```bash
git add backend2/services/jd/evaluator.py
git commit -m "feat(backend2): add JD evaluator layer (pure function)"
```
...
Expected: `OK: no graph imports in jd modules`
