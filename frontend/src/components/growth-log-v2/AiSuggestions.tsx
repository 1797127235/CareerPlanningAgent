...
export function AiSuggestions({ suggestions, onConvert }: AiSuggestionsProps) {
  return (
    <div className="mt-3 p-3 rounded-lg bg-blue-50/60 border border-blue-100">
      <p className="text-[12px] font-semibold text-blue-700 mb-2">AI 建议</p>
      <div className="space-y-2">
        {suggestions.map((s, idx) => (
          <div key={idx} className="flex items-start justify-between gap-3">
            <span className="text-[13px] text-slate-700 leading-relaxed flex-1">
              {idx + 1}. {s.text}
            </span>
            <button
              onClick={() => onConvert(s.text)}
              className="shrink-0 text-[11px] font-medium text-blue-600 hover:text-blue-800 cursor-pointer"
            >
              转为计划
            </button>
          </div>
