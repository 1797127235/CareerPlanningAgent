...
【技能家族亲和性】
- CV/深度学习背景（图像分割、目标检测、Mamba、CNN、Transformer、PyTorch、计算机视觉、医学图像）用户：优先 algorithm-engineer、ai-engineer、machine-learning、ai-data-scientist。不得推 developer-relations、technical-writer、product-manager（技能栈零交集）除非 job_target 明确写这些方向。图像分割+Mamba+PyTorch 是极强的 CV 信号，affinity 应给 algorithm-engineer 或 ai-engineer。
...
1. **技能栈与求职意向冲突检测（关键新增）**：
   检查用户的技能栈和 job_target 是否存在严重冲突：
   - 强技术信号定义：技能列表中包含 PyTorch/TensorFlow/深度学习/图像分割/Mamba/NeRF/计算机视觉/CNN/Transformer 等硬核技术，且有相关项目/实习经历
   - 如果用户有强技术信号但 job_target 是非技术方向（如"项目管理""产品经理""行政"），这属于**严重冲突**
   - 冲突时的处理规则：
     * 仍将 job_target 方向排在第1位（尊重用户主观选择）
     * 但必须在推荐列表中保留用户技能栈对应的技术方向（如算法工程师、AI工程师），排名紧随其后（第2-3位）
     * 禁止只推荐非技术方向而完全忽略技术方向——这是严重错误
     * 禁止推荐与技能栈和意向都无关的"第三方向"（如 CV 背景 + 项目管理意向 → 不得推 BI 分析师、数据分析师，这些与用户背景完全无关）
2. **primary_domain > 单项技能**：用户主方向是第二权重。
3. **career_signals 是区分相似岗位的关键信号**：
...
4. **工具技能不驱动主方向**：Docker/Linux/Git/CI/CD 是辅助技能，不能是 DevOps 匹配的主要依据（除非 primary_domain=系统/基础设施）。
5. **distinguishing_features 是决定因素**：当两个岗位技能相似时，对照每个岗位的"适合信号"和"不适合"来判断，而非单纯技能重叠度。
