...
async def main():
    async with async_playwright() as p:
        ...
        # Intercept APIs — HomePage mounts at "/" first and triggers /api/profiles,
        # which without interception hits the real backend, returns 401, and triggers
        # the client.ts login redirect that aborts the next goto. Route broadly.
        ...
        await page.route("**/api/growth-log/activity-pulse", lambda route: route.fulfill(status=200, content_type="application/json", body='{"current_streak_weeks":0,"total_records":0,"weeks":[]}'))
