...
export default function ExplorePage() {
  ...
  if (isLoading) {
    return (
      <main className="min-h-screen flex items-center justify-center px-6" style={{ background: 'var(--bg-paper)' }}>
        <p className="text-[13px]" style={{ color: 'var(--ink-3)' }}>加载岗位信息…</p>
      </main>
    )
  }
      ...
      >
        <div className="mx-auto max-w-[880px]">
          <header className="mb-10">
            <p className="text-[10px] font-bold uppercase tracking-[0.22em] mb-3" style={{ color: 'var(--ink-3)' }}>
              EXPLORE
            </p>
            <h1 className="text-[32px] md:text-[40px] font-black leading-[1.1] tracking-[-0.02em]" style={{ fontFamily: 'var(--font-serif)', color: 'var(--ink-1)' }}>
              并排看两个方向。
            </h1>
            <p className="mt-3 text-[13px] max-w-[56ch]" style={{ color: 'var(--ink-2)' }}>
          ...
          {leftNode?.contextual_narrative && rightNode?.contextual_narrative && (
            <>
              ...
              <div className="grid grid-cols-2 gap-6 pt-8" style={{ borderTop: '1px solid var(--line)' }}>
                <button
                  onClick={() => chooseAsTarget(leftNode.node_id)}
                  className="text-[13px] font-semibold py-3 px-4 transition-all duration-200 cursor-pointer"
                  style={{
                    color: 'var(--ink-1)',
