    ...
    return (
      <div className="flex items-center gap-2 mt-3">
        ...
        <button onClick={() => onSelect('debriefed' as ApplicationStatus)}
          className="text-[10px] text-[var(--ink-3)] hover:text-[var(--ink-2)] cursor-pointer transition-colors">
          撤销
        </button>
      </div>
    )
    ...
    <div className="mt-3">
      <div className="flex items-center">
        ...
        <button
          ...
          className="text-[10px] text-[var(--ink-3)] hover:text-[var(--ink-2)] cursor-pointer transition-colors shrink-0 pb-3.5"
        >
          标记结果
        </button>
      </div>
      ...
      {showOutcome && (
        <div className="flex items-center gap-1.5 mt-2 pl-0.5">
          ...
          <button onClick={() => setShowOutcome(false)}
            className="text-[10px] text-[var(--ink-3)] hover:text-[var(--ink-2)] cursor-pointer ml-1 transition-colors">
            取消
          </button>
        </div>
      )}
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col items-center shrink-0 w-14 pt-1 z-10">
          ...
          <span className="text-[9px] text-[var(--ink-3)] mt-1 tabular-nums text-center leading-tight">
            {dateLabel}
          </span>
        </div>

        {/* Card */}
        <div className="flex-1 group rounded-xl rounded-xl px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[13px] font-bold text-[var(--ink-1)]">{round.round}</span>
              {rating && (
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={handleAnalyzeRound} disabled={analyzing}
                className="p-1 rounded text-[var(--ink-3)] hover:text-purple-500 hover:bg-purple-50 transition-colors cursor-pointer"
                title="AI 复盘本轮">
                {analyzing ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
              </button>
              <button onClick={() => setConfirmDel(true)}
                className="p-1 rounded text-[var(--ink-3)] hover:text-red-500 hover:bg-red-50 transition-colors cursor-pointer">
              ...
              {pairs.map((pair, i) => (
                <div key={i}>
                  <div className="flex items-start gap-1.5">
                    ...
                    <p className="text-[12px] font-medium text-[var(--ink-1)] leading-relaxed flex-1">{pair.q}</p>
                    <button
                      onClick={() => handleAnalyzeQ(pair.q, pair.a)}
                      className="shrink-0 p-0.5 text-[var(--ink-3)] hover:text-purple-500 transition-colors cursor-pointer"
                      title="AI 分析这道题">
                  ...
                  {pair.a && pair.a !== '(未填写)' && (
                    <div className="flex items-start gap-1.5 pl-5 mt-0.5">
                      <span className="text-[10px] font-bold text-[var(--ink-3)] mt-0.5 shrink-0 w-5">A{i + 1}</span>
                      <p className="text-[11px] text-[var(--ink-2)] leading-relaxed">{pair.a}</p>
                    </div>
                  )}
                </div>
              ))}
          ...
          {round.ai_analysis && (
            <div className="mt-3 p-3 rounded-lg"
              ...
              {round.ai_analysis.overall && (
                <p className="text-[11px] text-[var(--ink-1)] leading-relaxed mb-1">{round.ai_analysis.overall}</p>
              )}
              ...
              {round.ai_analysis.weaknesses?.length > 0 && (
                <p className="text-[10px] text-[var(--ink-2)] mt-0.5">
...
function AddInterviewForm({ appId, company, position, onClose }: {
  ...
  const iCls = "w-full px-3.5 py-2.5 text-[12px] rounded-xl outline-none bg-[var(--bg-card)] border border-[var(--line)] focus:border-blue-400 focus:bg-white transition-colors"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <p className="text-[15px] font-bold text-[var(--ink-1)] mb-5">记录面试</p>
        <div className="space-y-4">

          {/* Round type */}
          <div>
            <p className="text-[11px] text-[var(--ink-3)] mb-1.5">轮次</p>
          ...
          <div className="flex gap-4 items-start">
            <div>
              <p className="text-[11px] text-[var(--ink-3)] mb-1.5">面试日期</p>
              <input type="date" value={date} onChange={e => setDate(e.target.value)}
                className="px-3 py-2 text-[12px] rounded-xl border border-[var(--line)] bg-[var(--bg-card)] outline-none focus:border-blue-400 cursor-pointer" />
            </div>
            <div>
              <p className="text-[11px] text-[var(--ink-3)] mb-1.5">发挥</p>
          ...
          {/* Q/A */}
          <div>
            <p className="text-[11px] text-[var(--ink-3)] mb-2">面试题目（选填）</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {pairs.length > 1 && (
                      <button onClick={() => setPairs(pairs.filter((_, idx) => idx !== i))}
                        className="p-1 text-[var(--ink-3)] hover:text-red-400 cursor-pointer transition-colors">
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button onClick={onClose}
              className="px-4 py-2.5 text-[13px] text-[var(--ink-2)] rounded-xl cursor-pointer border border-[var(--line)] hover:bg-[var(--bg-card)] transition-colors">
              取消
            </button>
          </div>
        </div>
  ...
  return (
    <div className="mt-6">
      <div className="flex items-center gap-3 mb-4">
        <span className="text-[13px] font-semibold text-[var(--ink-2)]">整体复盘</span>
        <div className="flex-1 h-px bg-[var(--line)]/60" />
        {saved && (
      ...
      <textarea
        ...
        rows={4}
        className="w-full px-4 py-3 text-[13px] rounded-2xl outline-none bg-white/60 border border-[var(--line)] focus:border-blue-400 focus:bg-white transition-colors resize-none leading-relaxed text-[var(--ink-1)] placeholder:text-[var(--ink-3)]"
      />
    ...
    return (
      <main className="min-h-screen pt-[64px]" style={{ background: 'var(--bg-paper)', color: 'var(--ink-1)' }}>
        ...
        <div className="max-w-[720px] mx-auto px-4 py-6 md:px-8 text-center">
          <p className="text-[var(--ink-2)] mb-4">无效的投递 ID</p>
    ...
    <main className="min-h-screen pt-[64px]" style={{ background: 'var(--bg-paper)', color: 'var(--ink-1)' }}>
      ...
      <div className="flex items-start gap-3 mb-5">
        <button onClick={() => navigate('/growth-log')}
          className="p-1.5 rounded-lg hover:bg-white/40 transition-colors cursor-pointer text-[var(--ink-3)] hover:text-[var(--ink-1)] mt-1">
          <ArrowLeft className="w-4 h-4" />
        </button>
        <div className="flex-1 min-w-0">
          <p
            className="text-[20px] font-bold text-[var(--ink-1)] truncate"
            style={{ viewTransitionName: `record-app-${appId}` } as React.CSSProperties}
          >
            {app.company || '未知公司'}
          </p>
          <p className="text-[13px] text-[var(--ink-2)] mt-0.5 truncate">{app.position || '未命名岗位'}</p>
      ...
      <div className="relative">
        {rounds.length > 0 && (
          <div className="absolute left-7 top-4 bottom-4 w-0.5 bg-[var(--line)]" />
        )}

        {/* Application start node */}
        <div className="relative flex items-start gap-0 mb-6">
          <div className="flex flex-col items-center shrink-0 w-14 pt-1 z-10">
            <div className="w-3.5 h-3.5 rounded-full border-2 border-white shadow-sm bg-[var(--line)]" />
            <span className="text-[9px] text-[var(--ink-3)] mt-1 tabular-nums text-center leading-tight">
              {fmtDate(app.created_at)}
            </span>
          </div>
          <div className="flex-1 rounded-xl rounded-xl px-4 py-3">
            <span className="text-[12px] font-semibold text-[var(--ink-1)]">投递记录</span>
            {app.notes && <p className="text-[11px] text-[var(--ink-3)] mt-1">{app.notes}</p>}
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="space-y-4 pl-14">
            {[1, 2].map(i => <div key={i} className="h-20 rounded-xl animate-pulse rounded-xl" />)}
          </div>
        ) : rounds.length === 0 ? (
      ...
      <div className="mt-4 flex justify-center">
        <button onClick={() => setConfirmDelete(true)}
          className="text-[11px] text-[var(--ink-3)] hover:text-red-500 cursor-pointer transition-colors px-3 py-1.5 rounded-lg hover:bg-red-50">
          删除这条投递记录
        </button>
      </div>
