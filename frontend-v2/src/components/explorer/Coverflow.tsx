...
const ZONE_CLS: Record<string, string> = {
  safe: 'bg-[#E8F0E8] text-[#3D5A3D]',
  transition: 'bg-[#F5ECD0] text-[#7A5C1A]',
  danger: 'bg-[#F0E0D6] text-[#7A3A1A]',
  leverage: 'bg-[#E0E8F0] text-[#3D4A6B]',
}
const ZONE_STRIP: Record<string, string> = {
  safe: 'from-[#5A7D5A] to-[#7A9D7A]',
  transition: 'from-[#B8860B] to-[#D4A84A]',
  danger: 'from-[#A0522D] to-[#C0724D]',
  leverage: 'from-[#4A6FA5] to-[#6A8FC5]',
}
const ZONE_DOT: Record<string, string> = { safe: '#5A7D5A', transition: '#B8860B', danger: '#A0522D', leverage: '#4A6FA5' }
const ZONE_RIBBON_DARK: Record<string, string> = { safe: '#526F59', transition: '#9A7B3D', danger: '#8B5E3C', leverage: '#4A6B8A' }
...
const TAG_CLS: Record<string, string> = {
  '最快': 'bg-[#F5E8E0] text-[var(--chestnut)] border-[#D9C4B8]',
  '最稳': 'bg-[#E8F0E8] text-[#5A7D5A] border-[#B8D4B8]',
  '高薪': 'bg-[#F5ECD0] text-[#B8860B] border-[#D4C49A]',
  '备选': 'bg-[var(--bg-paper)] text-[var(--ink-3)] border-[var(--line)]',
}
...
function drawRadar(canvas: HTMLCanvasElement, abilities: Record<string, number>, size: number, animate = false, target?: Record<string, number>) {
  ...
  function render(progress: number) {
    ...
    for (let ring = 1; ring <= 4; ring++) {
      ...
      ctx.strokeStyle = '#D9D4CC'; ctx.lineWidth = 0.5; ctx.stroke()
    }
    for (let i = 0; i < n; i++) {
      const a = (i / n) * Math.PI * 2 - Math.PI / 2
    ...
    ctx.fillStyle = `rgba(184,92,56,${0.12 * ease})`; ctx.fill()
    ctx.strokeStyle = '#B85C38'; ctx.lineWidth = 1.5; ctx.globalAlpha = ease; ctx.stroke(); ctx.globalAlpha = 1
    // Dots + labels
    for (let i = 0; i < n; i++) {
      const a = (i / n) * Math.PI * 2 - Math.PI / 2, v = vals[i] / 100 * r * ease
      ctx.beginPath(); ctx.arc(cx + Math.cos(a) * v, cy + Math.sin(a) * v, 2.5, 0, Math.PI * 2)
      ctx.fillStyle = '#B85C38'; ctx.globalAlpha = ease; ctx.fill(); ctx.globalAlpha = 1
      if (progress > 0.4) {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="absolute top-4 left-4 right-4 z-30 flex items-start gap-3">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-[12px] font-semibold transition-all cursor-pointer ${
                zoneFilter === f.key
                  ...
                  : 'bg-white/60 backdrop-blur-sm border border-white/50 text-[var(--ink-2)] hover:bg-white/80'
              }`}
            >
              {f.label}
              {f.key === null && <span className="ml-1 text-[var(--ink-3)] font-normal">{filteredNodes.length}</span>}
            </button>
          ))}
          {fromNodeId && (
            <button
              ...
              title="回到我的位置"
              className="flex items-center gap-1 px-3 py-1.5 rounded-full text-[12px] font-semibold bg-white/60 backdrop-blur-sm border border-white/50 text-[var(--ink-2)] hover:bg-white/80 transition-all cursor-pointer"
            >
              <MapPin className="w-3.5 h-3.5 text-[var(--chestnut)]" />
              我的位置
            </button>
          )}
          {targetNodeId && (
            <button
              ...
              title="跳到目标岗位"
              className="flex items-center gap-1 px-3 py-1.5 rounded-full text-[12px] font-semibold bg-[#F5ECD0]/80 backdrop-blur-sm border border-amber-200/60 text-[#B8860B] hover:bg-[#F0E0B8]/80 transition-all cursor-pointer"
            >
        ...
        <div className="flex-1 flex justify-center min-w-0">
          <div ref={searchRef} className="relative w-full max-w-[360px]">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--ink-3)] pointer-events-none" />
            <input
              type="text"
              value={searchQuery}
              ...
              placeholder="搜索岗位..."
              className="w-full pl-10 pr-10 py-3 rounded-[20px] text-[13px] font-medium text-[var(--text-1)] bg-white/[0.38] backdrop-blur-[24px] backdrop-saturate-[140%] border-[1.5px] border-white/50 outline-none transition-all focus:bg-white/60 focus:border-[var(--chestnut)]/30 focus:shadow-[0_8px_32px_rgba(107,62,46,.10)] placeholder:text-[var(--text-3)]"
            ...
            {searchQuery && (
              <button onClick={() => { setSearchQuery(''); setSearchResults([]); setSearchOpen(false) }}
                ...
                <X className="w-4 h-4 text-[var(--ink-3)] hover:text-[var(--ink-2)]" />
              </button>
            )}
            {searchOpen && searchResults.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white/80 backdrop-blur-xl border-[1.5px] border-white/60 rounded-[20px] max-h-60 overflow-y-auto z-40"
                style={{ boxShadow: '0 8px 32px rgba(184,92,56,.10)' }}>
                {searchResults.map(r => (
                  <button key={r.node_id} onClick={() => handleSearchSelect(r.node_id)}
                    className="w-full text-left px-5 py-3 hover:bg-[var(--chestnut)]/[0.06] transition-colors flex items-center gap-2.5 cursor-pointer first:rounded-t-[20px] last:rounded-b-[20px]">
                    <span className="w-2 h-2 rounded-full shrink-0" style={{ background: ZONE_DOT[r.zone] ?? ZONE_DOT[DEFAULT_ZONE] }} />
                    <span className="text-[13px] font-semibold text-[var(--ink-1)]">{r.label}</span>
                    <span className="text-[11px] text-[var(--ink-3)] ml-auto">{r.role_family}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        {profiles && profiles.length > 1 && onProfileChange && (
          <div className="flex items-center gap-1.5 bg-white/70 backdrop-blur-sm border border-white/60 rounded-full px-3 py-1.5 shrink-0">
            <span className="text-[11px] text-[var(--ink-3)] shrink-0">画像</span>
            <select
              value={activeProfileId ?? ''}
              onChange={e => onProfileChange(Number(e.target.value))}
              className="text-[12px] font-semibold text-[var(--ink-1)] bg-transparent border-none outline-none cursor-pointer max-w-[120px]"
            >
              {profiles.map(p => (
        ...
        {filteredNodes.map((node, i) => {
          ...
          return (
            ...
            >
              ...
              {isLocateTarget && (
                <div
                  className="absolute -top-10 left-1/2 -translate-x-1/2 z-10 px-4 py-1.5 rounded-full bg-[var(--chestnut)] text-white text-[12px] font-bold whitespace-nowrap shadow-lg"
                  style={{
              ...
              {/* ── FRONT ── */}
              <div
                className="absolute inset-0 rounded-t-[20px] bg-[var(--bg-card)] border border-[var(--line)] p-6 flex flex-col"
                style={{
                  backfaceVisibility: 'hidden',
                  /* clipPath disabled: 3D transform breaks it in Chrome */
                  boxShadow: `
                    inset 0 2px 3px -1px rgba(0,0,0,0.03),
                    ${isLocateTarget
              ...
              >
                {/* Bookmark ribbon */}
                <div
                  className="absolute -top-2 left-8 w-9 h-12 flex items-start justify-center pt-1.5 text-[10px] font-semibold tracking-wider rounded-t-md"
                  style={{
                    background: `linear-gradient(180deg, rgba(255,255,255,0.14), rgba(255,255,255,0)), linear-gradient(180deg, ${ZONE_DOT[node.zone] ?? ZONE_DOT[DEFAULT_ZONE]} 0%, ${ZONE_RIBBON_DARK[node.zone] ?? ZONE_RIBBON_DARK[DEFAULT_ZONE]} 100%)`,
                    boxShadow: `
                      inset 1px 0 0 rgba(255,255,255,0.28),
                      inset -1px 0 0 rgba(0,0,0,0.08),
                      0 1px 1px rgba(60,60,40,0.12),
                      0 6px 14px rgba(80,60,40,0.12)
                    `,
                ...
                <div className="flex justify-between items-start mb-2 mt-1">
                  <div className="flex-1 min-w-0 mr-2">
                    <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                      ...
                      {isMyPosition && (
                        <span className="flex items-center gap-0.5 text-[10px] font-bold px-2 py-0.5 rounded-md bg-[#EAD4C8] text-[var(--chestnut)]">
                          <MapPin className="w-2.5 h-2.5" />当前
                        </span>
                      )}
                      {isMyTarget && (
                        <span className="flex items-center gap-0.5 text-[10px] font-bold px-2 py-0.5 rounded-md bg-[#F0E0B8] text-[#B8860B]">
                          <Target className="w-2.5 h-2.5" />目标
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[12px] text-[var(--ink-3)]">{ROLE_FAMILY_LABEL[node.role_family] ?? node.role_family}</span>
                    </div>
                  </div>
                ...
                {isCenter && node.must_skills && node.must_skills.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-2">
                    {node.must_skills.slice(0, 4).map(s => (
                      <span key={s} className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-[var(--bg-paper-2)] text-[var(--ink-2)] border border-[var(--line)]">{s}</span>
                    ))}
                  </div>
                )}
                ...
                {isCenter && node.contextual_narrative && (
                  <Link
                    ...
                    className="text-[11px] text-[var(--ink-3)] hover:text-[var(--chestnut)] mb-2 inline-block"
                  >
                    跟别的方向对比看看 →
                  </Link>
                )}
                ...
                {isCenter ? (
                  <div className="flex items-center justify-between mt-auto pt-1 gap-2">
                    <button
                      ...
                      className="text-[11px] font-semibold text-[var(--ink-3)] hover:text-[var(--chestnut)] bg-[var(--bg-paper)] hover:bg-[#F5E8E0] px-2.5 py-1 rounded-lg cursor-pointer transition-colors"
                    >
                      详情
                    </button>
                    <span className="text-[11px] text-[var(--ink-3)] flex-1 text-center">点击翻转看路径</span>
                    {profileId && (
                      <button
                        onClick={e => openDirectGoal(node, e)}
                        className="text-[11px] font-bold text-[var(--chestnut)] bg-[#F5E8E0] hover:bg-[#EAD4C8] px-2.5 py-1 rounded-lg cursor-pointer transition-colors"
                      >
                  ...
                  <div className="text-center text-[11px] text-[var(--ink-3)] mt-auto">点击卡片翻转查看路径</div>
                )}
                ...
                <div
                  ...
                  style={{
                    ...
                    borderTop: '14px solid var(--bg-card)',
                  }}
                />
              </div>
              ...
              {/* ── BACK ── */}
              <div
                className="absolute inset-0 rounded-t-[20px] bg-[var(--bg-card)] border border-[var(--line)]"
                style={{
                  backfaceVisibility: 'hidden',
              ...
              >
                ...
                <div className="mb-5 mt-1">
                  <div className="text-[13px] font-bold text-[var(--ink-3)] flex items-center gap-1.5 mb-3">
                    ...
                    {Array.from({ length: maxLevel }, (_, lv) => {
                      ...
                      return (
                        <div key={lv} className={`flex-1 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold transition-all ${
                          isCurrent ? 'bg-[var(--chestnut)] text-white ring-2 ring-[var(--blue)]/30 scale-110' :
                          lvNum < node.career_level ? 'bg-[var(--chestnut)]/10 text-[var(--chestnut)]' : 'bg-[var(--bg-paper-2)] text-[var(--ink-3)]'
                        }`}>
                          L{lvNum}
                        </div>
                      )
                ...
                <div>
                  ...
                  {routesLoading && isCenter ? (
                    <div className="text-[12px] text-[var(--ink-3)] py-4 text-center">加载中...</div>
                  ) : currentRoutes.length > 0 && isCenter ? (
                    <div className="space-y-2">
                      {currentRoutes.map(route => {
                        const tag = route.tag || ''
                        const tagCls = TAG_CLS[tag] ?? 'bg-[var(--bg-paper)] text-[var(--ink-3)] border-[var(--line)]'
                        return (
                          <div key={route.target_node_id}
                            onClick={e => { e.stopPropagation(); showCompare(route) }}
                            className="flex items-center gap-2.5 p-2.5 rounded-xl cursor-pointer border border-transparent hover:bg-[var(--chestnut)]/[0.06] hover:border-[var(--chestnut)]/20 transition-all">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5 mb-0.5">
                                {tag && (
                                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${tagCls}`}>{tag}</span>
                                )}
                                <span className="text-[13px] font-semibold text-[var(--ink-1)] truncate">{route.target_label}</span>
                              </div>
                            ...
                            <div className="flex flex-col items-end gap-0.5 shrink-0">
                              <span className={`text-[11px] font-bold ${(route.safety_gain ?? 0) >= 0 ? 'text-[#5A7D5A]' : 'text-[#A0522D]'}`}>
                                {(route.safety_gain ?? 0) >= 0 ? '+' : ''}{route.safety_gain ?? 0}%
                              </span>
                              <span className="text-[9px] text-[var(--ink-3)]">安全增益</span>
                            </div>
                          </div>
                        )
                      })}
                    ...
                    <div className="text-[12px] text-[var(--ink-3)] py-4 text-center">暂无转换路径</div>
                  ) : null}
                </div>

                <div className="text-center text-[11px] text-[var(--ink-3)] mt-4">点击翻转回正面</div>
      ...
      <div className="flex items-center justify-center gap-3 pb-5">
        <button onClick={() => navigate(currentIdx - 1)} disabled={currentIdx === 0}
          className="w-7 h-7 rounded-full bg-white/70 backdrop-blur border border-white/50 flex items-center justify-center text-[var(--ink-3)] hover:text-[var(--chestnut)] transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-default"
          style={{ boxShadow: '0 2px 8px rgba(0,0,0,.04)' }}>
          <ChevronLeft className="w-3.5 h-3.5" />
        </button>
        <div className="bg-white/70 backdrop-blur border border-white/50 rounded-full px-4 py-1.5 text-[12px] font-semibold text-[var(--ink-2)] tabular-nums"
          style={{ boxShadow: '0 2px 8px rgba(0,0,0,.04)' }}>
          <span className="text-[var(--chestnut)]">{currentIdx + 1}</span>
          <span className="text-[var(--ink-3)] mx-1">/</span>
          <span>{filteredNodes.length}</span>
          {zoneFilter && <span className="text-[var(--ink-3)] ml-1 font-normal">· {ZONE_LABEL[zoneFilter]}</span>}
        </div>
        <button onClick={() => navigate(currentIdx + 1)} disabled={currentIdx >= filteredNodes.length - 1}
          className="w-7 h-7 rounded-full bg-white/70 backdrop-blur border border-white/50 flex items-center justify-center text-[var(--ink-3)] hover:text-[var(--chestnut)] transition-colors cursor-pointer disabled:opacity-30 disabled:cursor-default"
          style={{ boxShadow: '0 2px 8px rgba(0,0,0,.04)' }}>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-6"
          ...
          <div className="glass-static max-w-[480px] w-full p-6 sm:p-8 rounded-[24px] shadow-2xl relative"
            ...
            <button onClick={() => setCompareData(null)}
              className="absolute top-5 right-5 text-[var(--ink-3)] hover:text-[var(--ink-2)] p-2 rounded-full hover:bg-white/50 cursor-pointer transition-colors">
              <X className="w-5 h-5" />
            </button>

            <div className="flex items-center gap-3 mb-8 mt-2 relative z-10">
              <h3 className="text-[18px] font-extrabold text-[var(--ink-1)]">{compareData.from.label}</h3>
              <span className="text-[var(--chestnut)] text-[20px] font-extrabold">→</span>
              <h3 className="text-[18px] font-extrabold text-[var(--ink-1)]">{compareData.to.label}</h3>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-6 relative z-10">
              <div className="bg-white/20 p-4 rounded-2xl border border-white/30 flex flex-col items-center justify-center text-center shadow-sm">
                <div className="text-[11px] font-bold text-[var(--ink-3)] mb-1">技能差距</div>
                <div className="text-[30px] font-black text-[var(--chestnut)] leading-none mb-1 drop-shadow-sm">
                  {compareData.routes.gap_skills.length}
                </div>
                <div className="text-[10px] text-[var(--ink-3)] font-semibold">项</div>
              </div>

              <div className="bg-white/20 p-4 rounded-2xl border border-white/30 flex flex-col items-center justify-center text-center shadow-sm">
                <div className="text-[11px] font-bold text-[var(--ink-3)] mb-1">安全增益</div>
                <div className={`text-[24px] font-black leading-none mb-1 drop-shadow-sm ${compareData.routes.safety_gain >= 0 ? 'text-[#5A7D5A]' : 'text-rose-600'}`}>
                  {compareData.routes.safety_gain >= 0 ? '+' : ''}{compareData.routes.safety_gain}%
                </div>
                <div className="text-[10px] text-[var(--ink-3)] font-semibold">
                  {compareData.routes.safety_gain >= 0 ? '风险降低' : '风险上升'}
                </div>
              </div>

              <div className="bg-white/20 p-4 rounded-2xl border border-white/30 flex flex-col items-center justify-center text-center shadow-sm">
                <div className="text-[11px] font-bold text-[var(--ink-3)] mb-1">学习时长</div>
                ...
                <div className="text-[10px] text-[var(--ink-3)] font-semibold">小时</div>
              </div>
            </div>
            ...
            {compareData.routes.gap_skills.length > 0 && (
              <div className="mb-8 relative z-10">
                <h4 className="text-[14px] font-bold text-[var(--ink-2)] mb-4 flex items-center gap-2">
                  核心补充技能
                </h4>
            ...
            {goalPhase === 'done' ? (
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-5">
                  <CheckCircle2 className="w-4 h-4 text-[#5A7D5A] shrink-0" />
                  <p className="text-[13px] font-bold text-[#5A7D5A]">
                ...
                <div className="grid grid-cols-2 gap-4 mb-5">
                  ...
                  {/* Gap skills */}
                  <div>
                    <p className="text-[11px] font-bold text-[var(--ink-3)] mb-2 uppercase tracking-wide">能力差距</p>
                    <div className="flex flex-wrap gap-1.5">
                      {compareData.routes.gap_skills.slice(0, 5).map(s => (
                        <span key={s} className="text-[10px] bg-[#F5ECD0] text-[#B8860B] border border-amber-200 px-2 py-0.5 rounded-md font-semibold">{s}</span>
                      ))}
                      {compareData.routes.gap_skills.length > 5 && (
                        <span className="text-[10px] text-[var(--ink-3)] self-center">+{compareData.routes.gap_skills.length - 5}</span>
                      )}
                    </div>
                    <p className="text-[11px] text-[var(--ink-3)] mt-3">
                      需补 <span className="font-bold text-[var(--ink-2)]">{compareData.routes.gap_skills.length}</span> 项
                      {compareData.routes.estimated_hours > 0 && (
                        <> · 约 <span className="font-bold text-[var(--ink-2)]">{compareData.routes.estimated_hours}h</span></>
                      )}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <button
                    ...
                    className="w-full bg-[var(--chestnut)] text-white py-3 rounded-xl text-[13px] font-bold cursor-pointer transition-all shadow-md shadow-blue-500/20 flex items-center justify-center gap-2 hover:opacity-90"
                  >
                    去成长档案追踪
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="py-2.5 rounded-xl text-[12px] font-medium text-[var(--ink-2)] bg-white/60 border border-[var(--line)] cursor-pointer hover:bg-white/80 transition-all"
                    >
                      查看能力画像
                    </button>
                    <button
                      onClick={() => setCompareData(null)}
                      className="py-2.5 rounded-xl text-[12px] font-medium text-[var(--ink-3)] bg-white/40 border border-[var(--line)] cursor-pointer hover:bg-white/60 transition-all"
                    >
                      继续探索
                    </button>
                  </div>
              ...
              <div className="relative z-10 space-y-3">
                ...
                <div className="bg-[#F5ECD0] border border-amber-200 rounded-xl px-4 py-3">
                  <p className="text-[12.5px] font-semibold text-amber-800 mb-0.5">确认设定目标？</p>
                  <p className="text-[11.5px] text-[#B8860B]">设定后不可更改，请确认选择「{compareData.routes.target_label ?? compareData.to.label}」作为职业目标。</p>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => { setGoalPhase('idle'); setGoalError(null) }}
                    className="flex-1 py-3 rounded-xl text-[13px] font-medium text-[var(--ink-2)] bg-white/60 border border-[var(--line)] cursor-pointer hover:bg-white/80 transition-all"
                  >
                    取消
                  </button>
                  <button
                    onClick={handleSetGoal}
                    className="flex-1 py-3 rounded-xl text-[13px] font-bold text-white bg-[var(--chestnut)] cursor-pointer hover:opacity-90 transition-all shadow-md shadow-blue-500/20"
                  >
                    确认锁定目标
                  </button>
                </div>
              ...
              <div className="relative z-10 flex gap-2">
                <button
                  ...
                  className="flex-1 py-3.5 rounded-xl text-[13px] font-medium text-[var(--ink-2)] bg-white/60 border border-[var(--line)] cursor-pointer hover:bg-white/80 transition-all flex items-center justify-center gap-1.5"
                >
                  前往查看
                  <ArrowRight className="w-3.5 h-3.5" />
                </button>
                {!profileId ? (
                  <button
                    disabled
                    className="flex-1 py-3.5 rounded-xl text-[13px] font-medium text-[var(--ink-3)] bg-[var(--bg-paper-2)] border border-[var(--line)] flex items-center justify-center cursor-not-allowed"
                  >
                    先建立画像
                  </button>
                ) : (
                  <button
                    ...
                    className="flex-1 py-3.5 rounded-xl text-[13px] font-bold text-white bg-[var(--chestnut)] cursor-pointer hover:opacity-90 transition-all shadow-md shadow-blue-500/20 flex items-center justify-center gap-1.5"
                  >
                    设为目标
                    <ArrowRight className="w-3.5 h-3.5" />
                  </button>
          ...
          <div className="glass-static max-w-[380px] w-full p-6 sm:p-8 rounded-[24px] shadow-2xl relative"
            ...
            {directGoalPhase === 'done' ? (
              <div className="space-y-4">
                <div className="flex items-center gap-2.5 bg-[#E8F0E8] border border-emerald-200 rounded-xl px-4 py-3">
                  <CheckCircle2 className="w-5 h-5 text-[#5A7D5A] shrink-0" />
                  <div>
                    <p className="text-[13px] font-bold text-emerald-800">目标已设定</p>
                    <p className="text-[11.5px] text-[#5A7D5A] mt-0.5">{directGoalNode.label}</p>
                  </div>
                </div>
                <button
                  onClick={() => routerNavigate('/growth-log')}
                  className="w-full bg-[var(--chestnut)] text-white py-3.5 rounded-xl text-[14px] font-bold cursor-pointer transition-all shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2 hover:opacity-90"
                >
                  去成长档案追踪
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl text-[13px] font-medium text-[var(--ink-3)] bg-white/40 border border-[var(--line)] cursor-pointer hover:bg-white/60 transition-all"
                >
                  稍后再说
                </button>
              </div>
              ...
              <div className="space-y-5">
                <div>
                  <h3 className="text-[18px] font-extrabold text-[var(--ink-1)] mb-1">设定职业目标</h3>
                  <p className="text-[12px] text-[var(--ink-3)]">可随时更改目标</p>
                </div>

                <div className="bg-white/40 rounded-2xl p-4 border border-white/50">
                  <div className="text-[18px] font-extrabold text-[var(--ink-1)]">{directGoalNode.label}</div>
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    {directGoalNode.role_family && (
                      <span className="text-[11px] text-[var(--ink-3)]">{ROLE_FAMILY_LABEL[directGoalNode.role_family] ?? directGoalNode.role_family}</span>
                    )}
                  </div>
                </div>

                <p className="text-[12px] text-[var(--ink-3)] bg-[var(--bg-paper)] rounded-xl px-4 py-3">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-3 rounded-xl text-[13px] font-medium text-[var(--ink-2)] bg-white/60 border border-[var(--line)] cursor-pointer hover:bg-white/80 transition-all disabled:opacity-50"
                  >
                    取消
                  </button>
                  <button
                    ...
                    disabled={directGoalPhase === 'saving'}
                    className="flex-1 py-3 rounded-xl text-[13px] font-bold text-white bg-[var(--chestnut)] cursor-pointer hover:opacity-90 transition-all shadow-md shadow-blue-500/20 disabled:opacity-60"
                  >
